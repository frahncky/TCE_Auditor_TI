...
% Compatibilidade com mapas já escritos. O diagrama passa a ser acompanhado
% de uma rotina explícita de revisão ativa; novos capítulos devem usar \mapafuncional.
...
\end{tikzpicture}
\end{center}
\textbf{Como revisar este mapa:} percorra cada ramo partindo do conceito central e explique, sem consultar o texto, (i) a definição, (ii) a relação com os ramos vizinhos e (iii) uma situação em que a banca poderia trocar os conceitos. Em seguida, transforme cada ligação em uma pergunta ``por quê?'' ou ``quando?''. Se não conseguir explicar uma seta ou contraste, retorne à teoria antes de resolver as questões.
\end{mapabox}
}
